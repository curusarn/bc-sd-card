...
def mailsWithOrg(results, group_res):
    mainOrgs = []
    maxOrgScore = 0
    for org in group_res:
        if not maxOrgScore:
            maxOrgScore = org[1]
        if org[1] + 1 < maxOrgScore:
            break
        mainOrgs.append(org[0])
    ...
    for mail in results:
        ...
        for ent in mail:
            ...
            if ent[1] + 1 < maxMailScore:
                break
            if ent[0] in mainOrgs:
                mailCount += 1
                break

    return mailCount
                
def mailsWithOrgLite(results, group_res):
...
for domain in domains:
    ...
    mailsMainOrgPercentage = mailsWithOrg(results, grouped_res) * 1.0 / len(results)
    ...
    domain_results.append((domain, mean))
